  ...
  '../../node_modules/.pnpm/nodemailer@8.0.5/node_modules/nodemailer/package.json'(
    ...
    module2.exports = {
      ...
      scripts: {
        ...
        'test:syntax':
          'docker run --rm -v "$PWD:/app:ro" -w /app node:6-alpine node test/syntax-compat.js',
      },
      repository: {
        type: 'git',
...
var require_shared = __commonJS({
  '../../node_modules/.pnpm/nodemailer@8.0.5/node_modules/nodemailer/lib/shared/index.js'(
    ...
    module2.exports.callbackPromise = (resolve2, reject) =>
      function () {
  ...
  '../../node_modules/.pnpm/nodemailer@8.0.5/node_modules/nodemailer/lib/mime-funcs/index.js'(
    ...
    module2.exports = {
      ...
      encodeWord(data, mimeWordEncoding, maxLength) {
        ...
        if (
          maxLength &&
          (mimeWordEncoding !== 'B' ? encodedStr : base64.encode(data)).length > maxLength
        ) {
          if (mimeWordEncoding === 'Q') {
      ...
      checkHeaders(data) {
        ...
        for (let i = 0, len = this.lastBytes.length + data.length; i < len; i++) {
          ...
          if (chr === 10 && i) {
            ...
            const pr2 =
              i > 1 ? (i - 2 < lblen ? this.lastBytes[i - 2] : data[i - 2 - lblen]) : false;
            if (pr1 === 10) {
                ...
                if (value2 && value2.url) {
                  if (key.toLowerCase().trim() === 'id') {
                    ...
                    return (
                      ...
                      this._formatListUrl(value2.url).replace(/^<[^:]+\/{,2}/, '')
                    );
                  }
...
var SMTP_FROM = process.env.SMTP_FROM ?? SMTP_USER ?? 'noreply@piramid.com';
var EXTRA_NOTIFY_EMAILS = (process.env.NOTIFY_EMAILS ?? '')
  .split(',')
  .map((e) => e.trim())
  .filter(Boolean);
...
async function getAdminEmails() {
  ...
  return [.../* @__PURE__ */ new Set([...dbEmails, ...EXTRA_NOTIFY_EMAILS])];
}
...
function emailTemplate(title, rows, bodyText) {
  const rowsHtml = rows
    .map(
      (r) => `
      <tr>
...
async function notifyDueReturn(items) {
  ...
  await Promise.all(
    items.map(async (item) => {
      ...
      const facts = [
        {
          name: '\u0E2D\u0E38\u0E1B\u0E01\u0E23\u0E13\u0E4C',
          value: `${item.modelCode} \u2014 ${item.productName}`,
        },
